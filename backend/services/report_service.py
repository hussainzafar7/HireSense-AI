...
def generate_pdf_report(session, questions, payload=None):
    if payload is None:
        strengths = []
        weaknesses = []
        scores = []
        ...
        ats_score = float(session.get("ats_score", 0))
        overall = round((avg_score * 0.65) + (ats_score * 0.35), 1) if questions else round(ats_score, 1)
        communication = round(min(100, avg_score * 0.8 + 20), 1) if questions else 0
        payload = {
    ...
    title_style = ParagraphStyle("Title", parent=styles["Title"], textColor=colors.HexColor("#e94560"), fontSize=22)
    heading_style = ParagraphStyle("Heading", parent=styles["Heading2"], textColor=colors.HexColor("#16213e"))
    story = [Paragraph("HireSense AI Interview Report", title_style), Spacer(1, 0.15 * inch)]
    story.append(Paragraph(f"Candidate: {resume.get('name', 'Candidate')}", styles["Normal"]))
    story.append(Paragraph(f"Generated: {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}", styles["Normal"]))
    story.append(Spacer(1, 0.2 * inch))
    summary_data = [
        ["Overall", "Technical", "Communication", "Recommendation"],
        [f"{payload['overall_score']}%", f"{payload['technical_score']}%", f"{payload['communication_score']}%", payload["recommendation"]]
    ]
    table = Table(summary_data, colWidths=[1.3 * inch, 1.3 * inch, 1.6 * inch, 1.8 * inch])
    ...
    for item in payload["weaknesses"]:
        story.append(Paragraph(f"\u2022 {item}", styles["Normal"]))
    story.append(Spacer(1, 0.15 * inch))
    story.append(Paragraph("Q&A Breakdown", heading_style))
